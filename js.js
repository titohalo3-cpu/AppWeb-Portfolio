/* ===== Animación de entrada (va primero para que nunca se quede bloqueada) ===== */
...
function pintarHoras() {
  ...
  document.querySelectorAll('.lang').forEach(entry => {
    ...
    const horas = entry.querySelector('.hours');
    const weekly = entry.querySelector('.weekly');
    if (!horas || !weekly) return;

    horas.textContent = total.toLocaleString(locale) + (idioma === 'es' ? ' h registradas' : ' h logged');
...
function pintarTema() {
  ...
  themeLabel.textContent = texto;
  themeBtn.setAttribute('aria-label', texto);
}
...
function cambiarIdioma(nuevo) {
  ...
  if (langLabel) langLabel.textContent = idioma === 'es' ? 'English' : 'Español';
  if (langBtn) langBtn.setAttribute('aria-label', idioma === 'es' ? 'Translate to English' : 'Traducir al español');
...
if (cyclesBtn && cyclesPop) {
  ...
  document.addEventListener('click', e => {
    if (!cyclesPop.contains(e.target) && !cyclesBtn.contains(e.target)) abrirCiclos(false);
  });
